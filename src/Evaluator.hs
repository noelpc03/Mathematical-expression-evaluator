...
import Binary (binaryToDecimal, decimalToBinary, evalBinOp, sumBin) 
...
evalBinary (BinAdd (Val x) (Val y)) = reverse (evalBinOp "+b" (show (round x)) (show (round y)))
evalBinary (BinSub (Val x) (Val y)) = reverse (evalBinOp "-b" (show (round x)) (show (round y)))
...
safeEval expr = do
    case expr of
        ...
        BinAdd _ _ -> return $ Left (evalBinary expr)
        BinSub _ _ -> return $ Left (evalBinary expr)
