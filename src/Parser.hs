...
hexAddHexExpr = do
    ...
    expr2 <- parseExpr
    char ')'
    return $ HexAdd expr1 expr2


hexSubHexExpr :: Parser Expr
hexSubHexExpr = do
    string "-h" 
    spaces
    char '('
...
binAddExpr = do
    ...
    expr2 <- parseExpr
    char ')'
    return $ BinAdd expr1 expr2

binSubExpr :: Parser Expr
binSubExpr = do
    string "-b" 
    spaces
    char '('
...
factor = Parsec.try binToDecExpr
    ...
    <|> Parsec.try binAddExpr
    <|> Parsec.try binSubExpr
    ...
    <|> Parsec.try hexAddHexExpr 
    <|> Parsec.try hexSubHexExpr
