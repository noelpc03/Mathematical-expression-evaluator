...
import Parser (Expr(..))
import Prelude hiding (tan, cos, sin, signum)
...
exprToString (Log base arg) = "log_" ++ exprToString base ++ "(" ++ exprToString arg ++ ")"
--exprToString (Abs x) = "abs(" ++ exprToString x ++ ")"
...
simplifyLog base arg
    | base == Val (P.exp 1) = Ln arg  -- base e
